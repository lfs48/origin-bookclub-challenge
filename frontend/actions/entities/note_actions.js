import * as notesAPIUtil from '../../util/api/books_api_util';
...
export const createNote = (formNote) => (dispatch) => {
    return booksAPIUtil.createNote(formNote).then(
        (note) => dispatch(receiveNote(note))
    );
};

export const updateNote = (formNote) => (dispatch) => {
    return booksAPIUtil.updateNote(formNote).then(
        (note) => dispatch(receiveNote(note))
    );
};

export const deleteNote = (id) => (dispatch) => {
    return booksAPIUtil.deleteNote(id).then(
